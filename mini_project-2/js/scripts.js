...
function showContent(sectionId) {
  ...
  `;
    ...
    <a class="w3-bar-item w3-button w3-hover-black" href="#summaryVN">About the course (Vietnamese) (*)</a>
    <a class="w3-bar-item w3-button w3-hover-black" href="#summaryEN">About the course (English) (*)</a>
...
function showAdminPage() {
  ...
  topMenus.forEach((menu, index) => {
    html += `<tr>
      <td>${menu.id}</td>
      <td>${menu.name || (menu.isHome ? 'Home (Logo)' : '')}</td>
      <td>
          <button class="w3-button w3-blue w3-small" onclick="viewAdminLeftMenu('${menu.id}')">View</button>
          ${!menu.isHome ? `<button class="w3-button w3-orange w3-small" onclick="editTopMenu(${index})">Edit</button>
                            <button class="w3-button w3-red w3-small" onclick="deleteTopMenu(${index})">Delete</button>` : ''}
        </td>
      </tr>`;
  });
...
function addTopMenu() {
  let id = prompt("Enter menu ID:");
  let name = prompt("Enter menu name:");
  if (id && name) {
...
function renderAdminLeftMenu() {
  ...
  items.forEach((item, index) => {
    html += `<tr>
            ...
            <td>${item.name}</td>
            <td>
                <button class="w3-button w3-blue w3-small" onclick="viewAdminLayout('${item.id}')">View (Layout)</button>
                <button class="w3-button w3-orange w3-small" onclick="editLeftMenu(${index})">Edit</button>
                <button class="w3-button w3-red w3-small" onclick="deleteLeftMenu(${index})">Delete</button>
            </td>
        </tr>`;
  });
  tbody.innerHTML = html;
...
function addLeftMenu() {
  let id = prompt("Enter left menu ID:");
  let name = prompt("Enter left menu name:");
  if (id && name) {
...
function editLeftMenu(index) {
  let leftMenus = JSON.parse(localStorage.getItem('leftMenus'));
  let newName = prompt('Edit left menu name:', leftMenus[currentTopMenuId][index].name);
  if (newName) {
...
function resetStudentInfo() {
  if (confirm("Are you sure you want to restore student information to default (CV, Projects, Community activities)?")) {
    ...
    contentLayouts['stu-projects'] = [{ id: 'content-projects', name: 'Project list', colSpan: 12 }];
    contentLayouts['stu-community'] = [{ id: 'content-community', name: 'Community activities', colSpan: 12 }];

    let contents = JSON.parse(localStorage.getItem('contents')) || {};
    contents['content-cv'] = `
      <h3 style="color: blue;">CV</h3>
      <img src="./assets/ava.jpg" alt="Student Photo" style="max-width: 150px; border-radius: 8px;">
      <p><b>Full name:</b> Ha Duc Duong</p>
      <p><b>MSSV:</b> 20235923</p>
    `;
    contents['content-projects'] = `
      <h3 style="color: blue;">Projects</h3>
      <ul>
        <li><b>Project 1:</b> Doctor Appointment - Using React, MongoDB</li>
        <li><b>Project 2:</b> AIMS - Using JavaFX, Supabase</li>
      </ul>
    `;
    contents['content-community'] = `
      <h3 style="color: blue;">Community activities</h3>
      <ul>
        <li><b>Activity 1:</b> Student volunteers help students in need</li>
        <li><b>Activity 2:</b> Participated in the freshman orientation week</li>
      </ul>
    `;
    localStorage.setItem('leftMenus', JSON.stringify(leftMenus));
...
function renderAdminLayout() {
  ...
  items.forEach((item, index) => {
    tableHtml += `<tr>
            <td>${item.id}</td>
            <td>${item.name}</td>
            <td>span ${item.colSpan}/12 columns</td>
            <td>
                <button class="w3-button w3-blue w3-small" onclick="viewAdminContent('${item.id}')">View/Write</button>
...
function addLayoutContent() {
  let id = prompt("Enter ID for the content item:");
  let name = prompt("Enter the name of the content item:");
  let colSpan = prompt("Enter the number of columns to display on the grid (from 1 to 12, for example: 12 is full width, 6 is 50%):", "12");
...
function editLayoutContent(index) {
  ...
  let newName = prompt("Edit content item name:", item.name);
  let newColSpan = prompt("Edit number of columns to display (1-12):", item.colSpan);
...
//Logic for Admin Contents (QuillJS Editor)
...
function renderContentPreview() {
  ...
  let html = '';
  items.forEach(item => {
    let contentHTML = contents[item.id] || '<p class="w3-text-grey w3-small">(No content)</p>';
